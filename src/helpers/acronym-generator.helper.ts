...
export class AcronymGenerator {
  static generate(word: string, shiftsTypes: ShiftTypesDict): ShiftCode {
    const re = /\s*(?:[;:\-+\s]|$)\s*/;
    const shiftAcrs = Object.values(shiftsTypes).map((shift) => shift.code);
    shiftAcrs.push("" as ShiftCode);
    let acronym = "" as ShiftCode;
    const wordSpliting = word.split(re);
    acronym = wordSpliting.reduce((acr, l) => {
    ...
    if (!shiftAcrs.includes(acronym)) {
      ...
      let acronymString = acronym.join("") as ShiftCode;
      let wordNumber = 0;
      while (shiftAcrs.includes(acronymString) && wordNumber < wordSpliting.length) {
        ...
        while (
          ...
          acronymString = acronym.join("") as ShiftCode;
          letterNumber += 1;
        }
        wordNumber += 1;
      }
